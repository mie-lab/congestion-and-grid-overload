...
from python_scripts.process_real_data_from_other_repo.parse_xml_file import parse_xml_file
...
def generate_sensor_locations_file_from_poly(poly: geometry.polygon = None):
    ...
    :return:
    """
    stations_dict = parse_xml_file("python_scripts/process_real_data_from_other_repo/xml_file.xml")
    print(stations_dict)
    ...
    if poly is not None:
        ...
        for sensor in sensors_lat_lon_dict.copy():
            sensor_point = geometry.Point(
                sensors_lat_lon_dict[sensor]["lon"],
                sensors_lat_lon_dict[sensor]["lat"],
            )
            if not poly.contains(sensor_point):
                del sensors_lat_lon_dict[sensor]

        for station in stations_dict.copy():
            station_point = geometry.Point(
                float(stations_dict[station]["lon"]),
                float(stations_dict[station]["lat"]),
            )
    ...
    with open("output_images/kepler_files/station_ids.csv", "w") as f:
        ...
        for station in stations_dict:
            csvwriter.writerow(
                [
                    stations_dict[station]["lat"],
                    stations_dict[station]["lon"],
                    station,
                ]
            )
...
if __name__ == "__main__":
    ...
    geo_big = {
        ...
        "coordinates": [
            [
                [-93.2065479545105, 44.89046586812256],
                [-93.2065479545105, 45.050773307785434],
                [-93.3191578141754, 45.050773307785434],
                [-93.3191578141754, 44.89046586812256],
                [-93.2065479545105, 44.89046586812256],
            ]
        ],
    }

    poly_big = geometry.Polygon([tuple(l) for l in geo_big["coordinates"][0]])

    poly = geometry.Polygon([tuple(l) for l in geo["coordinates"][0]])

    generate_sensor_locations_file_from_poly(poly_big)
